...
import {mockSignalR} from "./mockSignalR.js";
...
const ChatApp2 = () => {
    ...
    const startConnection = async () => {
        try {
            ...
            await mockSignalR.connection.start();
            setIsConnected(true);

            // Register for receiving messages
            mockSignalR.connection.on('ReceiveMessage', (message) => {
                setMessages(prevMessages => [...prevMessages, message]);
            });

            // Register for user joined notifications
            mockSignalR.connection.on('UserJoined', (user) => {
            ...
            mockSignalR.connection.on('UserLeft', (user) => {
            ...
            // Join the chat room
            await mockSignalR.connection.invoke('JoinChat', username);

            // Set initial messages and users
            setMessages(mockSignalR.messages);
            setUsers(mockSignalR.users);
    ...
    const sendMessage = async () => {
        ...
        try {
            await mockSignalR.connection.invoke('SendMessage', username, message);
            setMessage('');
        } catch (err) {
